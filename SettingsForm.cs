...
public class SettingsForm : Form
{
    ...
    public SettingsForm(AppSettings settings, bool allowModeChange = true, IReadOnlyList<string>? validationErrors = null)
    {
        ...
        if (settings.Mode != null)
            _modeCombo.SelectedItem = settings.Mode;
        ...
        int errorHeight = validationErrors?.Count > 0 ? 20 + (validationErrors.Count + 1) * 18 : 0;
    ...
    private void SaveSettings()
    {
        if (_modeCombo.SelectedItem != null)
            _settings.Mode = _modeCombo.SelectedItem.ToString();
