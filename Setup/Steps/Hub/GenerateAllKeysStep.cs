...
public class GenerateAllKeysStep : ISetupStep
{
    ...
    public GenerateAllKeysStep(SetupContext ctx) => _ctx = ctx;
    ...
    public async Task<SetupStepResult> RunAsync(IProgress<string> progress)
    {
        try
        {
            ...
            for (int i = 0; i < _ctx.TravelMachineNames.Count; i++)
            {
    ...
    {
        var (_, output) = await ProcessHelper.RunAsync("wg", "genkey");
        return output.Trim();
    }

    private static async Task<string> WgPubKey(string privateKey)
    {
        var psi = new ProcessStartInfo("wg", "pubkey")
        {
        ...
        await p.StandardInput.WriteLineAsync(privateKey);
    ...
    private string BuildHubConf(string hubPrivKey, List<(string Name, string PrivKey, string PubKey, string Ip)> travels)
    {
        ...
        foreach (var t in travels)
        {
            ...
            sb.AppendLine($"PublicKey = {t.PubKey}");
            sb.AppendLine($"AllowedIPs = {t.Ip}/32");
        }
        return sb.ToString();
    }
