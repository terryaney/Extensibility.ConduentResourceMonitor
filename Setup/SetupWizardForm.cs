...
public class SetupWizardForm : Form
{
    ...
    private Control BuildNavBar()
    {
        ...
        _btnPrev = new Button { Text = "← Previous", AutoSize = true, Height = 28 };
        _btnPrev.Click += (_, _) => Navigate(-1);

        _btnNext = new Button { Text = "Next →", AutoSize = true, Height = 28 };
        ...
        _btnPrev.Location = new Point(8, 8);

        bar.Controls.Add(_btnNext);
        _btnNext.Location = new Point(100, 8);

        bar.Controls.Add(_btnFinish);

        bar.Controls.Add(_lblProgress);
    ...
    private void SelectStep(int index)
    {
        ...
        badges.Add(step.IsManual ? "MANUAL" : "AUTO");
    ...
    {
        _btnFinish.Enabled = _statuses.Zip(_applicable, (s, a) =>
            !a || s == StepStatus.Complete || s == StepStatus.Skipped).All(x => x);
    }
